...
namespace DxPlay
{
    internal class DxPlay : IDisposable
    {
        ...
        private CaptureNullRenderer cnr;
        ...
        // This method detects shots in an image.
        // Providing it a ShotDetectionAlgorithm,
        // and it wil return a list with the starting time of each shot
        public List<KeyValuePair<int, int>> DetectShots(BasicShotDetectionAlgorithm algo)
        {
            if (cnr != null)
            {
                Console.WriteLine("Force quiting shotdetection for a new shotdetection!");
                cnr.Dispose();
            }
            // Create a new capture with null renderer, and add the shot detection algorithm
            cnr = new CaptureNullRenderer(m_sFileName);
            cnr.setShotDetectionAlgorithm(algo);

            Console.WriteLine("Started shotdetection");
            cnr.Start();
            cnr.WaitUntilDone(); // locking wait :(
            Console.WriteLine("Finished shotdetection");

            // Get the list of shots from the new capture
            List<KeyValuePair<int, int>> shots = cnr.getShots();
            Console.Write("Found "); Console.Write(shots.Count); Console.WriteLine(" shots");
            cnr.Dispose();
            cnr = null;

            // Export xml
            String input_file_name = m_sFileName.Replace(".avi", "");
            String output_file_name = input_file_name + "_" + algo.getAlgoNr().ToString() + ".xml";

            XmlWriter writer = XmlWriter.Create(output_file_name);
            writer.WriteStartElement("ShotDetection");
            writer.WriteAttributeString("file", m_sFileName);
            //element method
            writer.WriteStartElement("method");
            writer.WriteAttributeString("nr", algo.getAlgoNr().ToString());
            int i=1;
            algo.getParameters().ForEach(delegate(double parameter)
            {
                writer.WriteElementString("param" + i.ToString(), parameter.ToString());
                i++;
            });

            writer.WriteEndElement(); //end element method

            //element shots
            writer.WriteStartElement("shots");
            shots.ForEach(delegate(KeyValuePair<int, int> start_stop)
            {
                writer.WriteElementString("shot", start_stop.Key.ToString() + "-" + start_stop.Value.ToString());
            });
            writer.WriteEndElement(); //end element shots

            writer.WriteEndElement(); //end element ShotDetection
            writer.Close();

            return shots;
        }

        public Bitmap getShotOfFrame(int frame_nr)
        {
            double currentPosition = 0;

            // used to control the location of the video
            IMediaPosition imp = m_FilterGraph as IMediaPosition;
            IVideoFrameStep ivf = m_FilterGraph as IVideoFrameStep;

            // go to frame
            imp.put_CurrentPosition(0);
            ivf.Step(frame_nr, null);
            imp.get_CurrentPosition(out currentPosition);

            return IPToBmp(SnapShot());
        }

        public void playShot(long longStartFrame, long longEndFrame)
        {
            IMediaSeeking iMediaSeeking = m_FilterGraph as IMediaSeeking;
            iMediaSeeking.SetTimeFormat(TimeFormat.Frame);
            iMediaSeeking.SetPositions(
                longStartFrame, DirectShowLib.AMSeekingSeekingFlags.AbsolutePositioning,
                longEndFrame, DirectShowLib.AMSeekingSeekingFlags.AbsolutePositioning);
        }
        ...
        /*XML annotation*/
        public void annotateShot(int current_shot_begin, int current_shot_end, string info)
        {
            String shotXMLFilename = m_sFileName.Replace(".avi", ".xml");
            xmlInitShots(shotXMLFilename);
            xmlAppendShot(current_shot_begin, current_shot_end, info);
        }

        string shotFilename;

        void xmlInitShots(string shotXMLFilename)
        {
            if (! File.Exists(shotXMLFilename))
            {
                shotFilename = shotXMLFilename;
                XmlWriter writer = XmlWriter.Create(shotFilename);
                writer.WriteStartElement("Shots");
                writer.WriteEndElement(); //end element Shots
                writer.Close();
            }
        ...
        void xmlAppendShot(int begin, int end, string info)
        {
            XmlDocument doc = new XmlDocument();
            doc.Load(shotFilename);

            XmlNode shotInfo = doc["Shots"].AppendChild(doc.CreateElement("Shot"));
            shotInfo.AppendChild(doc.CreateElement("Begin")).AppendChild(doc.CreateTextNode(begin.ToString()));
            shotInfo.AppendChild(doc.CreateElement("End")).AppendChild(doc.CreateTextNode(end.ToString()));
            shotInfo.AppendChild(doc.CreateElement("Info")).AppendChild(doc.CreateTextNode(info));

            // Save edited xml document
            doc.Save(shotFilename);
