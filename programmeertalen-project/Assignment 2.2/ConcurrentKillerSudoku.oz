functor
   ...
   define
      ...
      fun {ParseArgs L}
         case L
         of nil then {System.showInfo 'Please pass two filenames for sudokus to solve!'}
                     {Application.exit 0}
                     nil
         [] _|nil then {System.showInfo 'Please pass two filenames for sudokus to solve!'}
                       {Application.exit 0}
                       nil
         [] _|_ then L
         end
   ...
   local P X F FR L LR Sudoku Sums SudokuR SumsR in
      X = {ParseArgs {Application.getArgs list('in'(type: string) 'in1'(type: string))}}
      F = {New Open.file init(url: X.1)}
      FR = {New Open.file init(url: X.2.1)}
      L = {F read(list:$)}
      LR = {FR read(list:$)}
      Sudoku#Sums = {ParseSudoku {String.tokens L &,} [nil] [nil]}
      SudokuR#SumsR = {ParseSudoku {String.tokens LR &,} [nil] [nil]}
      ...
      {ConstrainSums Sol SudokuR SumsR}

      {Barrier [proc {$} {Search.base.one {BuildConstraints Sol} P} end proc {$} {Search.base.one {BuildConstraints Sol} P} end]}
      {PrettyPrint P.1}
      {Application.exit 0}
   end
end
